...
//! are defined?
//! datatypes -> sim_t, base_sim_t, settings_t are defined in the file cs296_base.hpp. b2Vec2 is defined in b2Math.h present in Box2d 	source files.
namespace cs296
{
  int32 test_index = 0;
  ...
  void callbacks_t::resize_cb(int32 w, int32 h)
  {
	...
	//! The gluOrtho2D function defines a 2-D orthographic projection matrix.It is defined in Glu.h which is present in the openGl header files. It is present in compiled form in Glu32.lib which is one of library in openGl.
  ...
  {
    //! What are these?
	//! To avoid the warning of not using the variable x and y. As these values are unused so one needs to tell to ignore it.
    B2_NOT_USED(x);
    B2_NOT_USED(y);
	...
	//! If key has the value which doesn't belong to any of the above mentioned cases, then switch commands passes the control to default case and executes the command that are mentioned in default case.
    default:
      if (test)
	{
	  test->keyboard(key);
  ...
  void callbacks_t::mouse_cb(int32 button, int32 state, int32 x, int32 y)
  {
	...
	//! When mouse is pressed, dragged function mouse_motion_cb is called which takes the final co-ordinates of the mouse and take the reference co-ordinates from function mouse_cb and produces the motion between final and initial co-ordinates resulting in the movement of the things.
